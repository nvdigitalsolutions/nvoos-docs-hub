...
class NV_oOS_Docs_Hub_Settings {
	...
	public static function enqueue_admin_assets( $hook_suffix ) {
		...
		// Static script for the rebuild panel, dirty-state tracking, broken-link
		// fixes, and JSON export/import. Kept out of the page markup so the
		// settings page ships no inline <script> blocks (wp.org review feedback).
		wp_enqueue_script(
			'nvoos-dh-settings-page',
			NVOOS_DOCS_HUB_URL . 'assets/admin/settings-page.js',
			array(),
			NVOOS_DOCS_HUB_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'        => array(
					...
					'imported'            => __( 'Imported. A rebuild will run automatically.', 'nvoos-docs-hub' ),
					'importFailed'        => __( 'Import failed.', 'nvoos-docs-hub' ),
					'importRequestFailed' => __( 'Import request failed.', 'nvoos-docs-hub' ),
					'invalidJson'         => __( 'Invalid JSON file.', 'nvoos-docs-hub' ),
				),
			)
		);
	}
	...
	public static function render_page() {
		...
		<?php endif; ?>
			...
			<!-- Page scripts (rebuild panel, dirty-state tracking, broken-link fixes) are enqueued as assets/admin/settings-page.js. -->
			...
			<!-- Export / Import settings scripts are enqueued as assets/admin/settings-page.js. -->
